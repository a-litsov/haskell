...
-- Сайт курса: unnfp2019.wordpress.com

-- Домашнее задание 08.03.2019

-- 1. Прочитайте описание модуля Prelude, пропуская непонятные места.

-- Математическая запись типа: show :: ∀ a. a ∈ Show => a -> String
-- Синтакс Haskell: show :: Show a => a -> String
-- возвращает текстовое представление значения любого типа,
-- принадлежащего классу Show (см. лекцию 2).
...
-- Бесконечный список. В интерпретаторе нужно выводить только конечный фрагмент
allNumDigits :: String
allNumDigits = concatMap show [1..]
...
-- seqp означает "sum equals product"
-- Функция digitToInt определена в Data.Char
seqp :: Integer -> Bool
seqp n = let digits = map digitToInt (show n) in
  sum digits == product digits

-- Бесконечный список. В интерпретаторе  нужно выводить только конечный фрагмент
sumEqProd :: [Integer]
sumEqProd = [x | x <- [1..], seqp x]
...
numPowers :: [[Integer]]
numPowers = [[x^y | y <- [1..]] | x <- [1..]]

-- Для тестирования
pow x y = numPowers !! (x-1) !! (y-1)
